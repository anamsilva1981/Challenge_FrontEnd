...
export class ProductsService {
  ...
  private _productsSubject$ = new BehaviorSubject<ProductResponse>(
    productResponse
  );
  private _productsDetails!: Product;

  public readonly products$: Observable<ProductResponse> =
    this._productsSubject$.asObservable();

  public getAllProducts(): void {
    this._http
      .get<ProductResponse>(`${environment.API}/products`)
      .pipe(take(1))
      .subscribe((response) => {
  ...
  public getProductByFilter(filter: string): void {
    this._http
      .get<ProductResponse>(`${environment.API}/products/search?q=${filter}`)
      .pipe(take(1))
      .subscribe((response) => {
        this._productsSubject$.next(response);
      });
